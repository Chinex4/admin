...
const FundUserModal = () => {
  ...
  const selectedWallet = useMemo(() => {
    ...
    return wallets.find(
      (w) => String(w?.symbol || '').toUpperCase() === normalized
    );
  }, [wallets, symbol]);
  ...
  const selectedCoinPriceUsd = useMemo(() => {
    ...
    const matchedBalance = userBalances.find((entry) => {
      ...
      const entrySymbol = String(entry?.symbol || entry?.coin_symbol || '')
        ...
        .toLowerCase();
      return (
        (coinId && entryId === coinId) ||
        (normalizedSymbol && entrySymbol === normalizedSymbol)
      );
    });
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      />
                      <p className="muted-text text-xs mt-1">
                        {Number.isFinite(selectedCoinPriceUsd) &&
                        selectedCoinPriceUsd > 0
